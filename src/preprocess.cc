...
void Preprocess::dominate_simplify_v() {
	...
	while (!working_vertice.empty()) {
		while (!working_vertice.empty()) {
			...
			bool is_dominated = false; //whether v dominate u
			for (auto u : adjacency_matrix[v]) {
			...
			if (is_dominated) {
				...
				//decrease degree of neighbooring vertice, and update next_working_vertice 
				for (auto u : adjacency_matrix[v]) {
					vector<long>::size_type i = 0;
...
void Preprocess::rm_acnodes() {
	...
	if (vertice_map.find(adjacency_matrix.size()) != vertice_map.end()) { //ending
